...
For an values of alpha [0 - 1] and ensembles creates a set of replica lattices.
Specify run_number (folder name). 
Only supports action 'W' (Wilson action) for now.
...
def run_replica_simulation(Nt, Nx, Ny, Nz, beta, u0, n, s, Ncfg, Nhits, run_number, action, new_base_dir, alpha, ensemble):
    import os
    import gauge_latticeqcd as glqcd
    ...
    for alpha in alphas:
        for ensemble in ensembles:
            p = Process(target=run_replica_simulation,
                        args=(Nt, Nx, Ny, Nz, beta, u0, n, s,
                            Ncfg, Nhits, run_number, action, new_base_dir, alpha, ensemble))
            p.start()
            processes.append(p)
...
def main():
    ...
    analyse_action_differences(new_base_dir)
